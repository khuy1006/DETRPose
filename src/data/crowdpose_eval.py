...
class CrowdPoseEvaluator(object):
    def __init__(self, ann_file, iou_types, useCats=True):
        ...
        for iou_type in iou_types:
            if 'keypoints' in iou_type:
                self.coco_eval['keypoints'] = COCOeval(coco_gt, iouType=iou_type, sigmas=sigmas, use_area=False)
                self.coco_eval['keypoints'].useCats = useCats
            else:
    ...
    def cleanup(self):
        ...
        for iou_type in self.iou_types:
            if 'keypoints' in iou_type:
                self.coco_eval['keypoints'] = COCOeval(self.coco_gt, iouType=iou_type, sigmas=self.sigmas, use_area=False)
                self.coco_eval['keypoints'].useCats = self.useCats
            else:
                self.coco_eval[iou_type] = COCOeval(self.coco_gt, iouType=iou_type)
                self.coco_eval[iou_type].useCats = self.useCats
    ...
    def update(self, predictions):
        ...
        for iou_type in self.iou_types:
            ...
            if 'keypoints' in iou_type:
                coco_eval = self.coco_eval['keypoints']
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            ...
            if 'keypoints' in iou_type:
                create_common_coco_eval(self.coco_eval['keypoints'], self.img_ids, self.eval_imgs[iou_type])
            else:
